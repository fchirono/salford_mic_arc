...
class DSRawTimeSeries:
    """
    Class to read raw measurement data from Dewesoft files
    """
    ...
    # **********************************************************************
    def _read_mic_data(self, filename, mic_ch_names):
        """
        ...
        assert rec_length <= self.T*self.fs, "Actual hdf5 file data length is longer than 'T*fs' declared for this instance!"

        # *-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-
        # assert all mic channel names actually exist in h5file
        channel_names = list(h5file.keys())

        assert set(mic_ch_names).issubset(channel_names), "Channel named {} does not exist in this hdf5 file!"
    ...
    # **********************************************************************
    def _read_other_data(self, filename, other_ch_names):
        """
        ...
        assert set(other_ch_names).issubset(channel_names), "Channel named {} does not exist in this hdf5 file!"
    ...
    def estimate_peak_freq(self, f_low, f_high, Ndft=2**14):
        ...
        Uses a "spectral centroid" calculation to estimate the frequency,
        even if it falls in between frequency samples.
        """
    ...
    def calc_PSDs(self, Ndft=DEFAULT_NDFT, Noverlap=DEFAULT_NOVERLAP,
        ...
        """
        Calculates and outputs the PSDs of all channels. Optionally, start
        calculating at time 't0'.
        """
